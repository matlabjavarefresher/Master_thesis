function [minimum,fval] = anneal_1(loss, parent, options)
...
while ~finished;
    ...
    if itry >= max_try || success >= max_success;
        if T < minT || consec >= max_consec_rejections;
            finished = 1;
            total = total + itry; % Total is the number of function calls
            break;
        else
    ...
    if (newenergy < minF) % If new energy (diff in correl funcs) is very less
        parent = newparam; 
        oldenergy = newenergy;
        break
    end
    
    if (oldenergy-newenergy > 1e-6) % If energy has decreased
        parent = newparam;
        oldenergy = newenergy;
        success = success+1;
        consec = 0;
    else
        if (rand < exp( (oldenergy-newenergy)/(k*T) )); % If energy is not 
