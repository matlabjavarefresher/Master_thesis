...
image_top_left = [3; 5.5];
image_bottom_right = [1000; 1000];
...
max_tries = 100*num_shapes; % Max number of tries to place ns shapes on the image
...
A_no_overlap = 0;
A_always_allow = inf;
A_small_overlap = [90,800,5000,num_shapes];%for a=30,10;b=20,10;max_ol=3000;min_ol = 200 to 600 for pow=1 to 5 and min_ol=200 (100 for cutoff_sharp) for pow = 1 to 1.5
...
switch shape_type
    case 'smooth_convex'
        min_power = 1; max_power = 1.5;
        S=SimParamSet();
        S.add(SimParameter('a_dist',translate(BetaDistribution(2,2), 30, 10)));
...
save('pow_1_to_1point5_3rd_realisation','image_out');
