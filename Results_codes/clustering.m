...
theta= [zeros(max_try,1),zeros(max_try,1)];
%theta=theta_1;
p=2*ones(max_try,1);
...
for i=1:size(ov_er_lap,2)
    over_lap=ov_er_lap(i);
...
%     
%     title(['overlap = ' num2str(ov_er_lap(i))]);
%     
%     %get(xlabh,'Position')
%     %set(xlabh,'Position',get(xlabh,'Position') + ((1-i)==0)*[0 0.0042 0] + [0 .0025 0]);
%     xlabel('Chord length');
...
% xlabel('Inter pixel distance');
% ylabel('Correlation');
% legend (['theta = ' num2str(ov_er_lap(1))],['theta = ' num2str(ov_er_lap(2))]);
% 
% % 
% % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % 
